...
s1_34 = port{1}.uf.ref./ (port{3}.uf.inc - port{4}.uf.inc);
s2_34 = port{2}.uf.ref./ (port{3}.uf.inc - port{4}.uf.inc);
 
figure
xlim([freq(1) freq(end)]*1e-6);
grid on;
hold on;
plot(freq*1e-6,20*log10(abs(s1_34)),'r-','Linewidth',2);
plot(freq*1e-6,20*log10(abs(s2_34)),'b-','Linewidth',2);
l = legend('S_{1-34}','S_{2-34}','Location','Best');
set(l,'FontSize',12);
