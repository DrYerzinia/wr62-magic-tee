...
start=[a/-2 -10 2];
stop =[a/2  -5  b+2];
[CSX, port{4}] = AddRectWaveGuidePort( CSX, 0, 4, start, stop, 'y', b*unit, a*unit, 'TE01', 1);

Sim_Path = 'WR62_MagicTee';
Sim_CSX = 'WR62_MagicTee.xml';
...
s14 = port{1}.uf.ref./ port{4}.uf.inc;
s24 = port{2}.uf.ref./ port{4}.uf.inc;
s34 = port{3}.uf.ref./ port{4}.uf.inc;
s44 = port{4}.uf.ref./ port{4}.uf.inc;
 
figure
plot(freq*1e-6,20*log10(abs(s44)),'k-','Linewidth',2);
xlim([freq(1) freq(end)]*1e-6);
grid on;
hold on;
plot(freq*1e-6,20*log10(abs(s14)),'g--','Linewidth',2);
plot(freq*1e-6,20*log10(abs(s24)),'r--','Linewidth',2);
plot(freq*1e-6,20*log10(abs(s34)),'b--','Linewidth',2);
l = legend('S_{44}','S_{14}','S_{24}','S_{34}','Location','Best');
set(l,'FontSize',12);
